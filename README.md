...
这是一个可独立部署到 GitHub Pages 的静态 Web UI Demo。它从“宽主题 / 具体问题”双入口开始，展示 GOAI 文献 Agent 从问题定义到 B3—B7 的产品路径：主题收窄、ResearchBrief、自动文献研究、Gap 挑战和路线 C 交接。

它不是完整 Agent 后端。页面不调用模型、不访问数据库、不请求 Sciverse/MinerU，不需要 API Key。所有交互只保存在当前浏览器内存里，断网后仍可继续点击和输入。

## 功能

- 复刻参赛版沉浸式三栏研究工作台：研究旅程、中央对话主轴、右侧证据和 Formal 状态。
- 起点提供“我只有一个宽主题”和“我已经有具体研究问题”两条入口；两条路径最终汇合到同一 ResearchBrief 和 B5—B7。
- 宽入口通过三轮可自由输入的 QA 逐步补齐主题。可直接点击预设的科学家回答，快速复现“公开论文线索 → 主题收窄 → 候选择优”。
- 窄入口提供 8 项结构化表格，支持手工填写和一键载入演示样例；必填问题、材料和目标后即可创建 Mock Run。
...
首次细化双入口前的公开版本已保存在 Git 标签 `v0.1-public-baseline`，需要时可以从该标签恢复。
